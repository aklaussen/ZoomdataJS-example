...
var chart = barChart()
    ...
    .minimumColor(controller.variables["Minimum Color"])
    .neutralColor('#1f77b4')
    .maximumColor(controller.variables["Maximum Color"])
    .y(function(d) {
        return controller.metrics['Y Axis'].formatted(d);
    });

controller.update = function(data, progress) {
    // debugger;
    
    // workaround: add sorting logic here
    data.sort(function(a,b) { return a.group - b.group;});
    
    // data.forEach(function(item) {
    //     var group = item.group;
    //     var y = controller.metrics['Y Axis'].raw(item);
    //     // log data in the order it comes in
    //     console.log(group, y);
    // });
...
function barChart() {
    var instance = this,
        ...
        xAxis = d3.svg.axis().scale(xScale).orient("bottom").tickSize(0).ticks(0),
        yValue = function(d) { return d; },
        yAxis = d3.svg.axis().scale(yScale).orient("left").ticks(5).outerTickSize(6).tickFormat(yValue),
        currentBarPadding = 0.1,
    ...
    function chart(selection) {
        selection.each(function(data) {
            ...
            var barsGroup = gEnter.append("g")
                .attr("class", "bars-group");
            
            var brushEnter = gEnter.append("g")
                .attr("class", "brush")
                .call(brush);
            ...
            brushEnter.selectAll(".resize.w").append("image")
                ...
                // .attr("xlink:href", "https://raw.githubusercontent.com/Zoomdata/nhtsa-dashboard/master/app/images/left-gripper.png")
                .attr("width", 45)
                .attr("height", 40)
                .attr("x", -44);
            ...
            brushEnter.selectAll(".resize.e").append("image")
                ...
                // .attr("xlink:href", "https://raw.githubusercontent.com/Zoomdata/nhtsa-dashboard/master/app/images/right-gripper.png")
                .attr("width", 45)
                .attr("height", 40);

            var labelsGroup = gEnter.append("g")
            ...
            bars = g.select('.bars-group').selectAll("rect.bar")
                .data(data, groupKey);
            
            labels = g.select('.labels-group').selectAll("text.label")
                .data(data, groupKey);
            ...
            bars.enter()
                ...
                .attr("height", function(d) { return Math.abs(yScale(controller.metrics['Y Axis'].raw(d)) - yScale(0)); })
                .attr("x", function(d) {
                    return xScale(d.group)
                })
                .attr("y", function(d) {return controller.metrics['Y Axis'].raw(d) < 0 ? yScale(0) : yScale(controller.metrics['Y Axis'].raw(d)); });
    ...
    function updateAxisScales(data) {
        ...
        var yScaleDomain = [0, absMaxMetricValue];
        if(minMetricValue < 0) {
    ...
    function brushmove() {
        ...
        if (d3.event.mode === "move") {
            var d0 = Math.floor(extent0[0] / barWidth) * barWidth,
    ...
    function brushend() {
        ...
        if(isEmpty) {  
            ...
            var selectedYears = _.pluck(bars.filter('.selected').data(), 'group');
        
            controller.eventDispatcher.trigger('filter:years', selectedYears);
            console.log(selectedYears);
        }
    }
    
    var listener = {};
    _.extend(listener, controller.eventDispatcher);
    
    
    listener.on("filter:years:reset", function(msg) {
        d3.select(controller.element)
            .select(".brush")
            .transition()
            .call(brush.extent([93, 360]))
    ...
    function redrawAxis() {
        // Update x-axis group dimensions
        this.select(".x.axis")
            .transition()
            .duration(duration)
            .ease(ease)
...
// Remove default zooming with mousewheel
// $(controller.element).mousewheel(function() {
//     return false;
// });
